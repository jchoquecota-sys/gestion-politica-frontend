...
export function PublicNewsFeed({ noticias, isLoading }: PublicNewsFeedProps) {
  return (
    <section id="noticias" className="relative py-24 bg-background dark:bg-[#070d1a] overflow-hidden">
      {/* Decorative glow */}
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="rounded-2xl overflow-hidden border border-border dark:border-slate-800 animate-pulse">
                <div className="h-52 bg-muted dark:bg-slate-800" />
                <div className="p-6 space-y-3 bg-card dark:bg-slate-900">
                  <div className="h-4 bg-muted dark:bg-slate-800 rounded w-1/3" />
                  <div className="h-6 bg-muted dark:bg-slate-800 rounded" />
                  <div className="h-4 bg-muted dark:bg-slate-800 rounded" />
                </div>
              </div>
            ))}
          </div>
            ...
            {noticias.map((noticia, idx) => (
              ...
              >
                ...
                <div className="relative h-52 overflow-hidden bg-muted dark:bg-slate-800 flex-shrink-0">
                  ...
                  {noticia.tipo && (
                    <span className="absolute top-3 left-3 inline-flex items-center gap-1.5 bg-background/90 dark:bg-slate-900/90 text-foreground dark:text-slate-200 text-xs font-semibold px-3 py-1 rounded-full border border-border dark:border-slate-700 shadow-sm">
                ...
                <div className="flex-1 p-5 flex flex-col gap-2">
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground dark:text-slate-500 font-medium">
                    <Calendar className="h-3.5 w-3.5" />
                    {formatDate(noticia.fecha_actividad)}
                  </div>
                  <h3 className="text-base font-bold text-foreground dark:text-white leading-snug group-hover:text-primary transition-colors line-clamp-2">
                    {noticia.titulo}
                  </h3>
                  {noticia.descripcion && (
                    <p className="text-sm text-muted-foreground dark:text-slate-400 leading-relaxed line-clamp-3 flex-1">
                      {noticia.descripcion}
                    </p>
                  )}
                </div>
